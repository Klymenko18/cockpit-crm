...
def test_audit_written_on_open_and_update():
    ...
    update_entity(entity_uid=uid, display_name="Bob", entity_type="PERSON", change_ts=t0, actor="test")
    update_entity(entity_uid=uid, display_name="Bob B.", entity_type="PERSON", change_ts=t0, actor="test")

    actions = list(AuditLog.objects.values_list("action", flat=True))
    # We expect OPEN + CLOSE + OPEN
