...
class EntitiesAsOfView(APIView):
    ...
    @extend_schema(
        ...
        parameters=[
            OpenApiParameter("as_of", str, OpenApiParameter.QUERY, description="ISO date or datetime"),
            ...
            OpenApiParameter("detail_code", str, OpenApiParameter.QUERY, description="Filter entities that have this detail as-of"),
        ],
        responses={200: dict},
        tags=["entities"],
    )
    def get(self, request, *args, **kwargs):
        ...
        # Entity rows "open" at as_of: valid_from <= as_of < coalesce(valid_to, infinity)
        entities_qs = (
        ...
        if detail_code:
            # Ensure there exists a matching detail open at as_of
        ...
        # Prefetch details "open" at as_of for all entities in one pass
        ...
        return Response({"as_of": as_of, "count": len(result), "results": result}, status=status.HTTP_200_OK)
...
class DiffView(APIView):
    ...
    @extend_schema(
        ...
        parameters=[
            OpenApiParameter("from", str, OpenApiParameter.QUERY, description="ISO date/datetime (inclusive)"),
            OpenApiParameter("to", str, OpenApiParameter.QUERY, description="ISO date/datetime (exclusive)"),
        ],
        responses={200: dict},
        tags=["entities"],
    )
    def get(self, request, *args, **kwargs):
        ...
        if ts_to <= ts_from:
            return Response({"detail": "'to' must be greater than 'from'."}, status=status.HTTP_400_BAD_REQUEST)
        ...
        # Stable sort by timestamp then kind/op for deterministic output
